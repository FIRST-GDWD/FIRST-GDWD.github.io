function generateReportOnCSS(rawInput) {
    ...
    rawCSSInputElement.innerHTML = "";
    cleanedCSSInputElement.innerHTML = "";
    for (let i = 0; i < lineObjects.length; i++) {
        ...
        rawCSSInputElement.innerHTML += rawInputContent;
        ...
        cleanedCSSInputElement.innerHTML += cleanedInputContent;
    }
    ...
    const dirtyPercentCSSElement = document.getElementById("dirtyPercentCSS");
    const issuesListCSSElement = document.getElementById("issuesListCSS");

    let dirtyLines = lineObjects.filter(line => line.hasDirtyCode);
    if (dirtyCodeCountElement)
    ...
    if (badIndentationCountElement)
        badIndentationCountElement.innerHTML = badIndentationLines.length;
    dirtyPercentCSSElement.innerHTML = (dirtyLines.length / lineObjects.length * 100).toFixed(2);

    if (typeof updateCSSStats === "function") 
        updateCSSStats(dirtyLines.length, lineObjects.length);

    let newIssuesListHTML = "<h4>CSS</h4>";
    for (const line of dirtyLines) {
        newIssuesListHTML += 
            `<div class='issue'>${convertStringToEscapedHTML(line.errorMessage, false)}</div>`;
    }
    issuesListCSSElement.innerHTML = newIssuesListHTML;
