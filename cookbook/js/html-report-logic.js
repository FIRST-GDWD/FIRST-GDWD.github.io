function generateReportOnHTML(rawInput) {
    ...
    rawHTMLInputElement.innerHTML = "";
    cleanedHTMLInputElement.innerHTML = "";
    for (let i = 0; i < lineObjects.length; i++) {
        ...
        rawHTMLInputElement.innerHTML += rawInputContent;
        ...
        cleanedHTMLInputElement.innerHTML += cleanedInputContent;
    }
    ...
    const dirtyPercentHTMLElement = document.getElementById("dirtyPercentHTML");
    const issuesListHTMLElement = document.getElementById("issuesListHTML");
    ...
    dirtyPercentHTMLElement.innerHTML = (dirtyLines.length / lineObjects.length * 100).toFixed(2);

    if (typeof updateHTMLStats === "function") 
        updateHTMLStats(dirtyLines.length, lineObjects.length);

    let newIssuesListHTML = "<h4>HTML</h4>";
    for (const line of dirtyLines) {
        newIssuesListHTML += 
            `<div class='issue'>${convertStringToEscapedHTML(line.errorMessage, false)}</div>`;
    }
    issuesListHTMLElement.innerHTML = newIssuesListHTML;
